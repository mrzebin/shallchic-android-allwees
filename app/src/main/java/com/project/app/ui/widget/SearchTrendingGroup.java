...
public class SearchTrendingGroup<X extends TextView> extends ViewGroup {
    ...
    //选中的样式
    private int itemBGResPre = R.drawable.groupview_selected;
    private int itemTextColorPre = Color.parseColor("#EE643D");
    private int itemTextColorNor = Color.parseColor("#000000");


    private Context mContext;

    private List<TrendingSearchBean.SearchItem> mDatas = new ArrayList<>();
    ...
    public SearchTrendingGroup(Context context, AttributeSet attrs) {
        super(context,attrs);
        this.mContext = context;
    }

    public void addItemViews(List<TrendingSearchBean.SearchItem> texts, String mode) {
        mDatas = texts;
        removeAllViews();
        for (int i=0;i<texts.size();i++) {
            addItemView(i,texts.get(i).getWord(), mode);
        }
    }
    ...
    private void addItemView(int position,String text, String mode) {
        ...
        if(position == 0){
            childView.setBackgroundResource(itemBGResSel);
